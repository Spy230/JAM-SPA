...
// Переменные для автопереключения промо-акций
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Инициализация автопереключения промо-акций
    initPromoSlider();
...
const servicesData = {
    ...
    conversation: {
        ...
        fullDesc: "<p>Идеальный выбор, если хочется не только тела, но и души. Красивая, умная и невероятно нежная мастерица сначала поговорит с вами по душам за чашкой чая или бокалом вина: снимет напряжение, выслушает, подарит тепло и лёгкий флирт.</p><p>Эта программа создана для тех, кто ценит не только физическое, но и эмоциональное удовольствие. Приятная беседа, искренний интерес и настоящая близость — все это ждет вас.</p>"
    }
};
...
function initPromoSlider() {
    ...
    if (promoSlides.length <= 1) {
        console.log('Промо-слайдов меньше 2, автопереключение не требуется');
        return;
    }
    
    console.log('✅ Инициализация автопереключения промо-акций. Слайдов:', promoSlides.length);
    
    // Запускаем автопереключение каждые 5 секунд
    promoInterval = setInterval(switchPromoSlide, 5000);
}

function switchPromoSlide() {
    ...
    console.log('Переключение на промо-слайд:', currentPromoIndex);
}
    ...
    if (promoBanner) {
        promoBanner.addEventListener('mouseenter', function() {
            if (promoInterval) {
                ...
                console.log('Автопереключение промо остановлено (наведение мыши)');
            }
        });
        ...
        promoBanner.addEventListener('mouseleave', function() {
            if (promoSlides.length > 1) {
                promoInterval = setInterval(switchPromoSlide, 5000);
                console.log('Автопереключение промо возобновлено');
            }
        });
    }
});
